    ...
    for (td in c(.15))
    {
        for (h in c('lh', 'rh'))
        {
            ...
            for (i in 1:length(subjects))
            {
                ...
                fname <- paste('jacc_',ss,'_',td,'_',nom,'.ijk_',h,'_mesh140.1D', sep='')
            ...
            outname <- paste(grp_jacc_dir, 'subrun_group_jaccard_median_', h,'_mesh140_', td, '_', nom,'.1D', sep='')
