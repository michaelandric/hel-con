# -*- coding: utf-8 -*-
"""
Created on Tue Aug 11 12:16:51 2015
...
if __name__ == "__main__":
    ...
    if not os.path.exists(top_dir):
        ...
        sys.exit(1)
    group_Q_dir = os.path.join(top_dir, 'group_rnd_modularity')
    ...
    for thresh_dens in [.05, .10]:
        ...
        for i, ss in enumerate(subj_list):
            ...
            rnd_dir = os.path.join(top_dir, '%s/random' % ss)
            mod_dir = rnd_dir
            for session in range(1, 3):
                ...
                gr = ge.Graphs(ss, ts_name, thresh_dens, rnd_dir)
                q_fname = 'rnd_sess_%d_%s.dens_%s.Qval' % \
                    (session, ss, thresh_dens)
                qval, indx = gr.max_q(os.path.join(rnd_dir, q_fname))
                q_array[i, session-1] = qval
                nmod_fname = 'rnd_sess_%d_%s.dens_%s.nmods' % \
                    (session, ss, thresh_dens)
                nmods = gr.max_nmod(os.path.join(rnd_dir, nmod_fname), indx)
        ...
        out_qrep_fname = 'rnd_max_q_values_density%s_report.txt' % thresh_dens
        outqf = open(os.path.join(group_Q_dir, out_qrep_fname), 'w')
        outqf.write('\n'.join(q_out_report))
        outqf.close()
        out_nmodrep_fname = 'rnd_max_q_n_mods_density%s_report.txt' % thresh_dens
        outnmf = open(os.path.join(group_Q_dir, out_nmodrep_fname), 'w')
        outnmf.write('\n'.join(nmod_out_report))
        outnmf.close()
        q_array_fname = 'rnd_max_q_values_density%s_array.txt' % thresh_dens
        np.savetxt(os.path.join(group_Q_dir, q_array_fname),
                   q_array, fmt='%.4f')
        nmod_array_fname = 'rnd_max_q_n_mods_density%s_array.txt' % thresh_dens
