...
write.table(dat_matB, paste(out_dir, 'dat_matB_expcutoff.txt', sep=''),
            row.names=F, col.names=F, quote=F)
write.table(dat_matA, paste(out_dir, 'dat_matA_Truncpshape.txt', sep=''),
            row.names=F, col.names=F, quote=F)
...
plotaG <- barplot2(aG, beside = TRUE, ylim = ylimG, ylab = "Power law values", plot.grid = TRUE, xpd=F)
...
plotaB <- barplot2(aB, beside = TRUE, ylim = ylimB, ylab = "Exponent cutoff values", plot.grid = TRUE)
...
plotaA <- barplot2(aA, beside = TRUE, ylim = ylimA, ylab = "Trunc Power Shape values", plot.grid = TRUE)
